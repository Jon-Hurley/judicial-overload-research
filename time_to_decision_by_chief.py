...
def average_days_by_chief(df):
    # Chronological order of Chief Justices
    chief_order = ['Vinson', 'Warren', 'Burger', 'Rehnquist', 'Roberts']
    avg_days = df.groupby('chief')['daysToDecision'].mean()
    return avg_days.reindex(chief_order)

def plot_average_days(avg_days_by_chief):
    plt.figure(figsize=(10, 6))
    avg_days_by_chief.plot(kind='bar', color='skyblue')
    plt.title('Average Days to Decision by Chief Justice')
